...
or Windows Credential Locker, `@hypit/credential-store-platform` to keep each credential in the store its
host provides — this one where the host has no locker — or `@hypit/credential-store-env` for externally
